	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (bookingDAO.checkBooking(idevent, iduser) == true) {
				response.setStatus(HttpServletResponse.SC_PRECONDITION_FAILED);
				response.getWriter().println("Event already booked");
				return;
			}
			...
			if ((DateTimeComparator.getDateOnlyInstance().compare(event.getDate(), current) < 0)
				...
				response.setStatus(HttpServletResponse.SC_PRECONDITION_FAILED);
				response.getWriter().println("Event past not bookable");
				return;
			}
			...
			response.getWriter().println("Not possible to add booking");
			return;
		}
